...
const OZON_PERFORMANCE_CLIENT_ID = process.env.OZON_PERFORMANCE_CLIENT_ID;
const OZON_PERFORMANCE_CLIENT_SECRET = process.env.OZON_PERFORMANCE_CLIENT_SECRET;
...
function autoSaveFactIfUseful(text) {
...
function exportHistory() {
  ...
  const jsonPath = path.join(EXPORT_DIR, "history-" + timestamp + ".json");
  const mdPath = path.join(EXPORT_DIR, "history-" + timestamp + ".md");
...
app.get("/files", (req, res) => {
  res.json({
    files: loadFiles().map(f => ({ name: f.name, size: f.content.length }))
  });
});

app.get("/knowledge", (req, res) => {
  res.json({
    knowledge: loadKnowledge().map(f => ({ name: f.name, size: f.content.length }))
  });
});
...
app.post("/clear-profile", (req, res) => {
  saveProfile([]);
  res.json({ ok: true });
});
...
app.post("/chat-stream", async (req, res) => {
  try {
    ...
    const autoSaved = autoSaveFactIfUseful(userMessage);
...
async function askOllamaSimple(userMessage) {
  const memory = loadMemory();
  const profile = loadProfile();
  ...
  const data = await response.json();
  const reply = data.message?.content || "Нет ответа от модели.";
...
if (text.startsWith("/sheet ")) {
  try {
    ...
    await addRowToGoogleSheet(sheetName, row);
  ...
  return;
    ...
    try {
      ...
      const reply = await askOllamaSimple(text);
      await tgBot.sendMessage(chatId, reply.slice(0, 4000));
    } catch (err) {
  ...
  console.log("Telegram token not found. Telegram bot disabled.");
}
async function addRowToGoogleSheet(sheet, row) {
...
app.listen(PORT, () => {
  console.log("Bot started: http://127.0.0.1:" + PORT);
});
